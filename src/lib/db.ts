...
export async function getProjectInvestors(): Promise<ProjectInvestor[]> {
  ...
  const { data, error } = await supabase.from("project_investors").select("*");
  if (error) throw new Error(error.message);
...
export async function updateStartup(startup: Startup): Promise<void> {
  const { error } = await supabase.from("startups").update(startup).eq("startup_id", startup.startup_id);
  if (error) throw new Error(error.message);
  invalidateCache("startups");
}
...
export async function updateProject(project: Project): Promise<void> {
  const { error } = await supabase.from("projects").update(project).eq("project_id", project.project_id);
  if (error) throw new Error(error.message);
  invalidateCache("projects");
}
...
export async function updateInvestor(investor: Investor): Promise<void> {
  const { error } = await supabase.from("investors").update(investor).eq("investor_id", investor.investor_id);
  if (error) throw new Error(error.message);
  invalidateCache("investors");
}
...
export async function createProjectInvestor(pi: ProjectInvestor): Promise<void> {
  ...
  const { error } = await supabase.from("project_investors").insert(record);
  if (error) throw new Error(error.message);
...
export async function updateProjectInvestor(pi: ProjectInvestor): Promise<void> {
  ...
  const { error } = await supabase.from("project_investors").update(record).eq("link_id", pi.link_id);
  if (error) throw new Error(error.message);
  invalidateCache("project_investors");
}

export async function deleteProjectInvestor(linkId: string): Promise<void> {
  const { error } = await supabase.from("project_investors").delete().eq("link_id", linkId);
  if (error) throw new Error(error.message);
  invalidateCache("project_investors");
}
...
export async function createStartupInvestor(si: StartupInvestor): Promise<void> {
  const { error } = await supabase.from("startup_investors").insert(si);
  if (error) throw new Error(error.message);
  invalidateCache("startup_investors");
}

export async function updateStartupInvestor(si: StartupInvestor): Promise<void> {
  const { error } = await supabase.from("startup_investors").update(si).eq("link_id", si.link_id);
  if (error) throw new Error(error.message);
  invalidateCache("startup_investors");
}
...
export async function updateTeamMember(member: TeamMember): Promise<void> {
  const { error } = await supabase.from("team").update(member).eq("team_id", member.team_id);
  if (error) throw new Error(error.message);
...
export async function updateProjectNote(note: ProjectNote): Promise<void> {
  const { error } = await supabase.from("project_notes").update(note).eq("note_id", note.note_id);
  if (error) throw new Error(error.message);
  invalidateCache("project_notes");
}

export async function deleteProjectNote(noteId: string): Promise<void> {
  const { error } = await supabase.from("project_notes").delete().eq("note_id", noteId);
  if (error) throw new Error(error.message);
  invalidateCache("project_notes");
}
...
export async function updateMeeting(meeting: Meeting): Promise<void> {
  const { error } = await supabase.from("meetings").update(meeting).eq("meeting_id", meeting.meeting_id);
  if (error) throw new Error(error.message);
  invalidateCache("meetings");
}
